...
def new_comer(bot, update, user_data):
	...
	bot.restrict_chat_member(chat_id = group, user_id = id, can_send_messages = False, can_send_media_messages = False, can_send_other_messages = False, can_add_web_page_previews = False)
...
def confirm(bot, update, args, user_data):
	...
	if real == captcha:
		...
		bot.restrict_chat_member(chat_id = group, user_id = id, can_send_messages = True, can_send_media_messages = True, can_send_other_messages = True, can_add_web_page_previews = True)
		bot.send_message(chat_id = my, text = '%s: %s' % (name, str(uid)))
	else:
...
def bot_gift(bot, update, uid):
	if newbie(uid):
		status = transfer(uid, 23333, '幼兔娘的新人礼物w')
		if status == 0:
...
def trick(bot, update, chat_data, user_data):
	...
	try:
		...
		bot.send_photo(chat_id = group, photo = photo_id)
	except:
		pass
	try:
		sticker_id = update.effective_message.sticker.file_id
		bot.send_sticker(chat_id = group, sticker = sticker_id)
	except:
		pass
	return
...
# Handler
trick_handler = MessageHandler(Filters.chat(my), trick, pass_chat_data=True, pass_user_data=True)
